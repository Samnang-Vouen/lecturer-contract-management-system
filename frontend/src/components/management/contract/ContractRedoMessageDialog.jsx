...
function getLatestManagementRedoRequest(contract, requests) {
  const redoRequest = (requests || []).find(
    (row) => String(row?.requester_role || '').trim().toUpperCase() === 'MANAGEMENT'
  );
  ...
  const fallbackMessage = getManagementRedoMessage(contract);
  if (fallbackMessage) {
    return {
...
export default function ContractRedoMessageDialog({ open, onOpenChange, contract }) {
  ...
  useEffect(() => {
    let cancelled = false;
    const canShowMessage = hasManagementRedoMessage(contract);

    if (!open || !contract?.id || !canShowMessage) {
      setRequests([]);
      setError('');
      setLoading(false);
      return undefined;
    ...
    const loadRequests = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(fetchError?.response?.data?.message || 'Failed to load management message');
          setRequests([]);
        }
      } finally {
        if (!cancelled) {
  ...
  }, [contract?.contract_type, contract?.id, open]);

  const latestRequest = useMemo(
    () => getLatestManagementRedoRequest(contract, requests),
    [contract, requests]
  );
          ...
          {!loading && !error && latestRequest ? (
            <div className="space-y-3 rounded-xl border border-blue-200 bg-blue-50/60 p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-slate-900">Latest management message</div>
                  <div className="text-xs text-slate-500">
                    {submittedAt ? `Submitted on ${submittedAt}` : 'Submission date unavailable'}
                  </div>
                </div>
                <span className="inline-flex items-center rounded-full border border-blue-200 bg-white px-2.5 py-1 text-xs font-medium text-blue-700">
                  Waiting response
                </span>
              </div>
