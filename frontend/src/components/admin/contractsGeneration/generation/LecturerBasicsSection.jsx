...
export default function LecturerBasicsSection({ lecturers, dlgLecturerKey, handleLecturerChange, dlgErrors, dlgHourlyRate }) {
  const getLecturerDisplayName = (lecturer) => lecturer.name || lecturer.full_name_english || lecturer.full_name_khmer || '';

  const sortedLecturers = [...lecturers].sort((left, right) => (
    getLecturerDisplayName(left).localeCompare(getLecturerDisplayName(right), undefined, {
      sensitivity: 'base',
    })
  ));
      ...
      <div className="space-y-1 mb-4">
        ...
        <Select className="w-full cursor-pointer" value={dlgLecturerKey} onValueChange={handleLecturerChange} placeholder="Select lecturer name">
          {sortedLecturers.map((lecturer) => (
            <SelectItem key={lecturer.id} value={lecturer.id}>{getLecturerDisplayName(lecturer)}</SelectItem>
          ))}
        </Select>
