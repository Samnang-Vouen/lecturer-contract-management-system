...
function buildTeachingHoursByUserId(contracts) {
  ...
  (contracts || []).forEach((contract) => {
    ...
    if (!hoursByUserId.has(lecturerUserId)) {
      hoursByUserId.set(lecturerUserId, buildTermSlots());
    }
...
function buildContractRateMap(contracts, currentAcademicYear) {
  ...
  (contracts || []).forEach((contract) => {
    ...
    if (!existing || candidateTimestamp >= existing.timestamp) {
      contractRateMap.set(mapKey, {
        rate: toRoundedNumber(parsedRate, 2),
...
async function buildRateHourReport(currentAcademicYear) {
  ...
  const lecturers = await LecturerProfile.findAll({
    ...
    include: [
      {
        model: Candidate,
        attributes: ['id', 'hourlyRate', 'dept_id'],
        required: false,
      },
      {
        model: Department,
  ...
  const candidateDeptIds = Array.from(
    new Set(
      lecturers.map((lecturer) => lecturer?.Candidate?.dept_id).filter((value) => Number.isInteger(value))
    )
  );
  ...
  const candidateDepartmentMap = new Map(
    candidateDepartments.map((department) => [department.id, department])
  );

  const lecturerUserIds = Array.from(
    new Set(
      lecturers
        .map((lecturer) => Number(lecturer.user_id))
  ...
  const lecturerProfileIds = Array.from(
    new Set(
      lecturers
        .map((lecturer) => Number(lecturer.id))
        .filter((lecturerId) => Number.isInteger(lecturerId))
    )
  );

  const [currentYearCourseMappings, teachingContracts, historicalTeachingContracts, advisorContracts, currentYearEvaluations] = await Promise.all([
  ...
  const lecturerRows = lecturers.map((lecturer) => {
    ...
    const rateMap = rateAcademicYears.reduce((acc, academicYear) => {
      const match = (lecturer.HourlyRateHistories || []).find(
        (history) => history.academic_year === academicYear
      );
      const contractRate = contractRateMap.get(`${Number(lecturer.user_id)}:${academicYear}`)?.rate ?? null;
      acc[academicYear] =
    ...
    const nextYearHistory = (lecturer.HourlyRateHistories || []).find(
      (history) => history.academic_year === nextAcademicYear
    );
  ...
  }).filter((row) => row.department.id !== null || row.department.englishName || row.department.khmerName);
...
export const getHourlyRateReport = async (req, res) => {
  try {
    const academicYear = await resolveReportAcademicYear(req.query.academicYear);
    const report = await buildRateHourReport(academicYear);
...
export const updateHourlyRateReport = async (req, res) => {
  ...
  try {
    ...
    const editableIncreaseRate =
      increaseRate !== undefined ? increaseRate : rates?.[nextAcademicYear];
    ...
    const lecturerProfile = await LecturerProfile.findByPk(lecturerId, {
      include: [
        ...
        },
        {
          ...
          required: false,
          where: {
            academic_year: { [Op.in]: buildTrailingAcademicYears(normalizedCurrentAcademicYear, 3) },
          },
        },
      ],
      transaction,
    ...
    const rateMap = rateAcademicYears.reduce((acc, academicYear) => {
      ...
      const contractRate = contractRateMap.get(`${Number(lecturerProfile.user_id)}:${academicYear}`)?.rate ?? null;
    ...
    if (
      (rateMap[normalizedCurrentAcademicYear] === null || rateMap[normalizedCurrentAcademicYear] === undefined) &&
      lecturerProfile.Candidate?.hourlyRate !== null &&
      lecturerProfile.Candidate?.hourlyRate !== undefined
    ) {
      rateMap[normalizedCurrentAcademicYear] = toRoundedNumber(lecturerProfile.Candidate.hourlyRate, 2);
    }
    ...
    for (const academicYear of Object.keys(rates)) {
      if (!/^\d{4}-\d{4}$/.test(academicYear)) {
        ...
        return res.status(400).json({
          message: `Invalid academic year provided: ${academicYear}`,
        });
      }
      ...
      if (academicYear !== nextAcademicYear) {
        await transaction.rollback();
        return res.status(400).json({
          message: `Only ${nextAcademicYear} can be updated from this report`,
        });
      }
    }
    ...
    if (
      ...
      await transaction.rollback();
      return res.status(400).json({
        message: 'Increase rate must be a valid number',
      });
    }
    ...
    if (parsedIncreaseRate !== null && parsedIncreaseRate < 0) {
      await transaction.rollback();
      return res.status(400).json({
        message: 'Increase rate must be zero or greater',
      });
    }
    ...
    if (parsedIncreaseRate !== null && latestRate === null) {
      await transaction.rollback();
      return res.status(400).json({
        message: 'Cannot calculate the next academic-year rate because the latest hourly rate is missing',
      });
    }
    ...
    return res.status(200).json({
      message: 'Rate hour data updated successfully.',
    });
  } catch (err) {
    await transaction.rollback();
    return res.status(500).json({
