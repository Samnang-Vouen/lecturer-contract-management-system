...
// ---------------------------------------------------------------------------
// Service: getMyCoursesData
// ---------------------------------------------------------------------------

export async function getMyCoursesData(lecturerUserId, query) {
  const { term, academic_year, year_level, activeOnly = 'true' } = query;
...
// ---------------------------------------------------------------------------
// Service: getMyCourseMappingsData
// ---------------------------------------------------------------------------

export async function getMyCourseMappingsData(userId, query) {
  const { term, academic_year, year_level } = query;
  ...
  for (const r of rows) {
    ...
    if (tg > 0 && thNum !== null && thNum !== undefined) {
    ...
    if (lg > 0 && lhNum !== null && lhNum !== undefined) {
  ...
  const aggregated = Array.from(aggMap.values())
    ...
    .map((a) => {
      ...
      const parts = [];
      if (a.theory_groups > 0 && theoryHoursNum !== null && theoryHoursNum !== undefined) {
        parts.push(`Theory ${theoryHoursNum}h × ${a.theory_groups}`);
      }
      if (a.lab_groups > 0 && labHoursNum !== null && labHoursNum !== undefined) {
        parts.push(`Lab ${labHoursNum}h × ${a.lab_groups}`);
      }
      return {
        id: a.ids[0],
        ids: a.ids,
  ...
  // Resolve contract end dates and totals
  const includeWhere = {
  ...
  return aggregated.map((m) => {
    ...
    const contract_total_hours =
      contract_id !== null && contract_id !== undefined
        ? totalHoursByContractId.get(contract_id) || 0
        : null;
    return { ...m, contract_end_date, contract_id, contract_total_hours };
  });
}
