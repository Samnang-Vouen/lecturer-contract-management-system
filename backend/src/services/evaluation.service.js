...
function parseEvaluationGroupCell(rawValue) {
  ...
  if (!raw) return { specializationName: null, groupName: null };

  const canonicalMatch = raw.match(/^(.+?)\s+Group\s+(.+)$/i);
  if (canonicalMatch) {
    return {
      specializationName: canonicalizeEvaluationSpecializationName(canonicalMatch[1]),
      groupName: String(canonicalMatch[2] || '').trim() || null,
    };
  }
  ...
  if (compactMatch) {
    return {
      specializationName: canonicalizeEvaluationSpecializationName(compactMatch[1]),
      groupName: String(compactMatch[2] || '').trim() || null,
    };
  }

  return { specializationName: null, groupName: null };
}
...
function getComparableEvaluationGroupToken(value) {
  ...
  if (/^\d+[A-Z0-9-]*$/u.test(normalized) && !normalized.startsWith('G')) return `G${normalized}`;
  return normalized;
}
...
function buildEvaluationGroupLookup(groupNames) {
  ...
  (Array.isArray(groupNames) ? groupNames : []).forEach((groupName) => {
    ...
    if (comparable && !byComparable.has(comparable)) byComparable.set(comparable, raw);
  });
...
function resolveEvaluationGroupName(rawValue, lookup) {
  ...
  if (!raw) return null;
  const normalized = normalizeEvaluationGroupLookupValue(raw);
  if (lookup?.byNormalized?.has(normalized)) return lookup.byNormalized.get(normalized);
  const comparable = getComparableEvaluationGroupToken(raw);
  if (lookup?.byComparable?.has(comparable)) return lookup.byComparable.get(comparable);
  if (/^\d+$/u.test(raw) && lookup?.byComparable?.has(`G${raw}`)) return lookup.byComparable.get(`G${raw}`);
  return raw;
}
...
function resolveEvaluationUploadGroups(parsedRows, assignments, classGroupNames) {
  ...
  const normalizedParsedRows = parsedRows.map((item) => ({
    ...item,
    groupName: resolveEvaluationGroupName(item.rawGroupName, classGroupLookup) || item.rawGroupName,
  }));
  ...
  return { normalizedParsedRows, fileGroupNames, fileGroupLookup, resolvedAssignments };
}
...
function getEvaluationScopeDetails(courseMapping) {
  return {
    ...
    academicYear:
      String(courseMapping?.academic_year || courseMapping?.Class?.academic_year || '').trim() || null,
...
function resolveEvaluationCourseOption(options, fallbackCourseName = '-') {
  ...
  if (uniqueOptions.length > 1) return { mappingId: null, courseId: null, courseName: 'Multiple Courses' };
  return { mappingId: null, courseId: null, courseName: fallbackCourseName || '-' };
}
...
async function loadEvaluationCourseMappingLookup(
  ...
  const normalizedGroupKeys = new Set(
    (Array.isArray(groupNames) ? groupNames : [])
      .map((g) => normalizeEvaluationGroupLookupValue(g))
      .filter(Boolean)
  );

  if (!classId || !academicYear || !term || numericLecturerIds.length === 0) {
    return { mappings: [], byLecturerGroup: new Map() };
  }

  const mappings = await CourseMapping.findAll({
    where: { lecturer_profile_id: numericLecturerIds, class_id: classId, academic_year: academicYear, term },
    include: [
      { model: Group, attributes: ['id', 'name', 'class_id'], required: false },
      { model: Course, attributes: ['id', 'course_name'], required: false },
    ],
  ...
  mappings.forEach((mapping) => {
    ...
    if (!Number.isInteger(lecturerId) || lecturerId <= 0 || !groupName) return;
    const normalizedGroupKey = normalizeEvaluationGroupLookupValue(groupName);
    if (normalizedGroupKeys.size > 0 && !normalizedGroupKeys.has(normalizedGroupKey)) return;
  ...
  return { mappings, byLecturerGroup };
}

// ---------------------------------------------------------------------------
// Service: getEvaluationSummaryData
// ---------------------------------------------------------------------------

export async function getEvaluationSummaryData() {
    ...
    include: [
      {
        ...
        include: [
          { model: ClassModel, attributes: ['id', 'name', 'term', 'year_level', 'academic_year'] },
        ...
        include: [
          {
            ...
            include: [{ model: EvaluationResponse, attributes: ['rating'] }],
          },
        ],
      },
    ],
  ...
  evaluations.forEach((evaluation) => {
    evaluation.EvaluationSubmissions?.forEach((submission) => {
      submission.LecturerEvaluations?.forEach((lecturerEvaluation) => {
        if (lecturerEvaluation?.lecturer_id) lecturerIdSet.add(lecturerEvaluation.lecturer_id);
      });
    });
  });
  ...
  const userIds = lecturers.map((l) => l.user_id).filter((id) => Number.isInteger(id));
  ...
  for (const evaluation of evaluations) {
    ...
    const lecturerIdsInEvaluation = Array.from(
      new Set(
        evaluation.EvaluationSubmissions?.flatMap(
          (sub) => sub.LecturerEvaluations?.map((le) => le?.lecturer_id) || []
    ...
    const groupNamesInEvaluation = Array.from(
      new Set(
        evaluation.EvaluationSubmissions?.map((sub) =>
          String(sub?.group_name || '').trim()
        ).filter(Boolean) || []
      )
    );
    ...
    byLecturerCourse.forEach((lecturerData) => {
      ...
      const totalPoint = groupScores.length
        ? roundToOne(groupScores.reduce((acc, g) => acc + g.score, 0) / groupScores.length)
        : 0;
      ...
      summaryRows.push({
        ...
        lecturer_name: lecturerMap[lecturerData.lecturerId]?.name || `Lecturer ${lecturerData.lecturerId}`,
  ...
  return { data: summaryRows, total: summaryRows.length };
}

// ---------------------------------------------------------------------------
// Service: getEvaluationResultsData
// ---------------------------------------------------------------------------

export async function getEvaluationResultsData(evaluationId) {
        ...
        include: [
          {
            ...
            include: [{ model: EvaluationResponse, attributes: ['question_id', 'rating'] }],
          },
        ],
      },
    ],
  ...
  if (!evaluation) {
    throw new NotFoundError('Evaluation not found', { payload: { message: 'Evaluation not found' } });
  }
  ...
  evaluation.EvaluationSubmissions?.forEach((submission) => {
    ...
    submission.LecturerEvaluations?.forEach((lecEval) => {
      ...
      if (!groupedData[key].lecturer_evaluations[lecId]) {
        groupedData[key].lecturer_evaluations[lecId] = { lecturer_id: lecId, evaluations: [] };
      }
        ...
        include: [
          {
            ...
            attributes: ['id', 'name'],
            include: [{ model: Department, attributes: ['id', 'dept_name'] }],
          },
        ],
      },
      { model: Course, attributes: ['id', 'course_name'] },
      { model: LecturerProfile, attributes: ['id', 'full_name_english', 'title'] },
    ],
  });
  ...
  const lecturerMap = lecturers.reduce((acc, lec) => {
    acc[lec.id] = { name: lec.full_name_english, title: lec.title };
    return acc;
  }, {});

  const scope = getEvaluationScopeDetails(courseMapping);
  const lecturerIdsInEvaluation = lecturerIds
    .map((id) => Number(id))
    .filter((id) => Number.isInteger(id));
  ...
  if (courseMapping?.Class) {
    const groupNames = Object.values(groupedData).map((g) => g.group_name).filter(Boolean);
    if (groupNames.length > 0) {
      const groups = await Group.findAll({
        where: { name: groupNames, class_id: courseMapping.Class.id },
        attributes: ['name', 'num_of_student'],
      });
      const groupMap = groups.reduce((acc, g) => {
        acc[g.name] = g.num_of_student;
        return acc;
      }, {});
...
// ---------------------------------------------------------------------------
// Service: uploadEvaluationData
// ---------------------------------------------------------------------------

export async function uploadEvaluationData(filePath, body) {
  ...
  if (parsedRows.length === 0) {
    throw new ValidationError('No evaluation groups detected', {
      payload: {
  ...
  if (!specializationName) {
    throw new ValidationError('Cannot detect specialization', {
      payload: {
  ...
  if (lecturer_assignments) {
    try {
      ...
      throw new ValidationError('Invalid lecturer_assignments format', {
        payload: {
    ...
    if (!Array.isArray(requestedAssignments) || requestedAssignments.length === 0) {
      throw new ValidationError('Missing lecturer assignments', {
        payload: {
    ...
    if (lecturer_names) {
      ...
      try {
        ...
        throw new ValidationError('Invalid lecturer_names format', {
          payload: {
      ...
      if (!Array.isArray(lecturerNameArray) || lecturerNameArray.length === 0) {
        throw new ValidationError('Missing lecturer_names', {
          payload: {
      ...
      try {
        ...
        throw new ValidationError('Invalid lecturer_ids format', {
          payload: {
      ...
      if (!Array.isArray(lecturerIdArray) || lecturerIdArray.length === 0) {
        throw new ValidationError('Missing lecturer_ids', {
          payload: {
    ...
    throw new ValidationError('Missing lecturer information', {
      payload: {
  ...
  const normalizedAssignments = requestedAssignments
    .map((assignment, index) => {
      ...
      const groupNames = Array.isArray(assignment?.group_names)
        ? assignment.group_names.map((g) => String(g || '').trim()).filter(Boolean)
        : [];
      return {
  ...
  if (normalizedAssignments.some((a) => !a.lecturer_id && !a.lecturer_name)) {
    throw new ValidationError('Invalid lecturer assignments', {
      payload: {
  ...
  if (duplicateOrderIndexes.length > 0) {
    throw new ValidationError('Duplicate lecturer order', {
      payload: {
  ...
  try {
    const assignmentLecturerIds = normalizedAssignments
      .map((a) => a.lecturer_id)
      .filter((id) => Number.isInteger(id));
    const assignmentLecturerNames = normalizedAssignments.map((a) => a.lecturer_name).filter(Boolean);
    ...
    const lecturerByName = new Map(
      lecturers.map((l) => [String(l.full_name_english || '').trim(), l])
    );
    ...
    if (duplicateLecturerIds.length > 0) {
      await transaction.rollback();
      throw new ValidationError('Duplicate lecturers in upload order', {
        payload: {
    ...
    if (Number.isInteger(selectedClassId) && selectedClassId > 0) {
      ...
      if (selectedClass.specialization_id !== specialization.id) {
        await transaction.rollback();
        throw new ValidationError('Specialization mismatch', {
          payload: {
    ...
    if (course_mapping_id) {
      ...
      if (!Number.isInteger(courseMappingId) || courseMappingId <= 0) {
        await transaction.rollback();
        throw new ValidationError('Invalid course_mapping_id', {
          payload: {
      ...
      if (selectedClass && Number(courseMapping?.class_id) !== Number(selectedClass.id)) {
        await transaction.rollback();
        throw new ValidationError('Class mismatch', {
          payload: {
    ...
    if (!courseMapping) {
      if (selectedClass && selectedAcademicYear && selectedTerm) {
        ...
        const {
          ...
          selectedClassGroups.map((g) => g.name).filter(Boolean)
        );
        ...
        if (unknownAssignedGroups.length > 0) {
          await transaction.rollback();
          throw new ValidationError('Assigned groups not found in file', {
            payload: {
        ...
        const candidateMappings = await CourseMapping.findAll({
          where: {
            lecturer_profile_id: resolvedAssignments.map((a) => a.lecturer_id),
        ...
        const hasMissingAssignmentMapping = resolvedAssignments.some((assignment) => {
          const matchingMappings = candidateMappings
            .filter((m) => Number(m.lecturer_profile_id) === Number(assignment.lecturer_id))
            .filter((m) => assignment.group_names.includes(String(m?.Group?.name || '').trim()));
          return matchingMappings.length === 0;
        });
        if (hasMissingAssignmentMapping) {
          await transaction.rollback();
          throw new ValidationError('Course mapping assignment mismatch', {
            payload: {
        ...
        const representativeAssignment =
          resolvedAssignments.find((a) => a.group_names.length > 0) ||
        ...
        courseMapping =
          candidateMappings.find(
            (m) =>
              Number(m.lecturer_profile_id) === Number(representativeAssignment?.lecturer_id) &&
              (!representativeGroupName ||
                normalizeEvaluationGroupLookupValue(m?.Group?.name) ===
        ...
        courseMapping = await CourseMapping.findOne({
          where: { lecturer_profile_id: normalizedAssignments[0]?.lecturer_id },
          include: [
            {
              model: ClassModel,
    ...
    if (!courseMapping) {
      await transaction.rollback();
      throw new NotFoundError('Course mapping not found', {
        payload: {
    ...
    const { normalizedParsedRows, fileGroupNames, fileGroupLookup, resolvedAssignments } =
      resolvedGroupData;
    ...
    const lecturerIds = resolvedAssignments.map((a) => a.lecturer_id);
    ...
    const availableMappingGroupKeys = new Set(
      assignmentMappings
        .filter((m) => m?.lecturer_profile_id && m?.Group?.name)
        .map((m) => `${m.lecturer_profile_id}|${normalizeEvaluationGroupLookupValue(m.Group.name)}`)
    );

    for (const assignment of resolvedAssignments) {
      if (assignment.group_names.length === 0) {
        await transaction.rollback();
        throw new ValidationError('Missing group assignments', {
          payload: {
    ...
    const assignedGroups = new Set(
      resolvedAssignments.flatMap((a) => a.group_names.map((g) => String(g)))
    );
    const unassignedFileGroups = fileGroupNames.filter((g) => !assignedGroups.has(g));
    if (unassignedFileGroups.length > 0) {
      await transaction.rollback();
      throw new ValidationError('Unassigned groups in upload file', {
        payload: {
    ...
    const resolvedCourseNames = Array.from(
      new Set(
        assignmentMappings
          .filter((m) => {
            const key = `${m.lecturer_profile_id}|${normalizeEvaluationGroupLookupValue(m?.Group?.name)}`;
            return selectedAssignmentKeys.has(key);
          })
          .map((m) => String(m?.Course?.course_name || '').trim())
          .filter(Boolean)
      )
    );
    const resolvedCourseLabel =
    ...
    for (const parsedRow of normalizedParsedRows) {
      ...
      const submission = await EvaluationSubmission.create(
        { evaluation_id: evaluation.id, specialization: rowSpecialization, group_name },
        { transaction }
      );
      ...
      for (const assignment of lecturersForGroup) {
        ...
        const lecturerEval = await LecturerEvaluation.create(
          { submission_id: submission.id, lecturer_id: assignment.lecturer_id, comment },
          { transaction }
        );
    ...
    try {
      ...
      console.error('Failed to emit evaluation:uploaded socket event:', socketErr?.message || socketErr);
    }
    ...
    return {
      ...
      lecturer_assignments: resolvedAssignments.map((a) => ({
...
// ---------------------------------------------------------------------------
// Service: getLecturerEvaluationPDFBuffer
// ---------------------------------------------------------------------------
...
export async function getLecturerEvaluationPDFBuffer(evaluationId, lecturerId) {
  const evaluation = await Evaluation.findByPk(evaluationId, {
    include: [
      {
        model: EvaluationSubmission,
        ...
        include: [
          {
            model: Specialization,
            attributes: ['id', 'name'],
            include: [{ model: Department, attributes: ['id', 'dept_name', 'dept_name_khmer'] }],
          },
        ],
      },
      { model: Course, attributes: ['id', 'course_name'] },
  ...
  const lecturerGroupNames = Array.from(
    new Set(
      evaluation.EvaluationSubmissions?.filter(
        (sub) => sub.LecturerEvaluations && sub.LecturerEvaluations.length > 0
      )
        .map((sub) => sub.group_name)
        .filter(Boolean) || []
    )
  );
  const { byLecturerGroup } = await loadEvaluationCourseMappingLookup({
  ...
  if (allComments.length > 0) {
    commentTableRows = allComments
      .map(
        (comment, index) =>
          `          <tr>\n            <td>${index + 1}.&nbsp;&nbsp; ${escapeHtml(comment)}</td>\n          </tr>`
      )
      .join('\n');
  } else {
  ...
  try {
    ...
    const pdfBuffer = await page.pdf({
      ...
      margin: { top: '20mm', right: '15mm', left: '15mm' },
    });
    return pdfBuffer;
  } finally {
